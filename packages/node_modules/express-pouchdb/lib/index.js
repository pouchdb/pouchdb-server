...
module.exports = function (startPouchDB, opts) {
  ...
  app.use(function (req, res, next) {
    var regexFindDbNameInPath = /\/([^/?]*)/
    var dbName = decodeURIComponent(req.url.match(regexFindDbNameInPath)[1])

    if (!dbName) {
      return next();
    }

    // allow internal routes starting with _ like /_all_dbs
    if (dbName[0] === '_' && internalPaths.indexOf(dbName) !== -1) {
      return next()
    }
    ...
    utils.sendJSON(res, 400, {
      ...
      reason: "Name: '" + dbName + "'. Only lowercase characters (a-z), digits (0-9), and any of the characters _, $, (, ), +, -, and / are allowed. Must begin with a letter."
    });
  });
  ...
  // check for internal routes GET /_all_dbs
  app._router.stack.forEach(function (stack) {
    if (!stack.route) return;

    if (stack.route.path[1] === '_') {
      internalPaths.push(stack.route.path.match(regexFindDbNameInPath)[1]);
    }
  });
